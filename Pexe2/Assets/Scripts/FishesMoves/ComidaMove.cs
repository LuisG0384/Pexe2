...
public class ComidaMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isTargetDetected && Target != null)
        {
            ...
            if (yDiff > 0.5f) targetTilt = -maxTiltAngle; 
            else if (yDiff < -0.5f) targetTilt = maxTiltAngle; 

            
            transform.rotation = Quaternion.Euler(currentTilt, angle, 0f);

            
            Vector3 directionToFlee = (transform.position - Target.position).normalized;
            finalMove = directionToFlee * speed;

            
            if (targetTilt < 0) finalMove.y = verticalSpeed; 
            else if (targetTilt > 0) finalMove.y = -verticalSpeed; 
